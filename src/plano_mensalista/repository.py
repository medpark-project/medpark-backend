...
def get_plano(db: Session, plano_mensalista_id: int) -> model.PlanoMensalista | None:
    return db.query(model.PlanoMensalista).filter(model.PlanoMensalista.id == plano_mensalista_id).first()

def get_all_planos(db: Session, skip: int = 0, limit: int = 100) -> list[model.PlanoMensalista]:
    return db.query(model.PlanoMensalista).offset(skip).limit(limit).all()

def create_plano(db: Session, plano_mensalista: schema.PlanoMensalistaCreate) -> model.PlanoMensalista:
...
def update_plano(db: Session, db_plano_mensalista: model.PlanoMensalista, plano_mensalista: schema.PlanoMensalistaUpdate) -> model.PlanoMensalista:
...
def delete_plano(db: Session, db_plano_mensalista: model.PlanoMensalista) -> model.PlanoMensalista:
