...
def create_plano_mensalista(plano_mensalista: schema.PlanoMensalistaCreate, db: Session = Depends(get_db)):
...
def read_all_planos_mensalista(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def read_plano_mensalista(plano_mensalista_id: int, db: Session = Depends(get_db)):
    db_plano_mensalista = repository.get_plano(db, plano_mensalista_id=plano_mensalista_id)
    if db_plano_mensalista is None:
        raise HTTPException(status_code=404, detail="Plano de Mensalista não encontrado")
    return db_plano_mensalista


@router.put("/{plano_mensalista_id}", response_model=schema.PlanoMensalista)
def update_plano_mensalista(plano_mensalista_id: int, plano_mensalista: schema.PlanoMensalistaUpdate, db: Session = Depends(get_db)):
    db_plano_mensalista = repository.get_plano(db, plano_mensalista_id=plano_mensalista_id)
    if db_plano_mensalista is None:
        raise HTTPException(status_code=404, detail="Plano de Mensalista não encontrado")
    updated_plano = repository.update_plano(db=db, db_plano_mensalista=db_plano_mensalista, plano_mensalista=plano_mensalista)
...
def delete_plano_mensalista(plano_mensalista_id: int, db: Session = Depends(get_db)):
    ...
    if db_plano_mensalista is None:
        raise HTTPException(status_code=404, detail="Plano de Mensalista não encontrado")
    deleted_plano = repository.delete_plano(db=db, db_plano_mensalista=db_plano_mensalista)
