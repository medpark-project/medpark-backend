...
def test_read_all_tipos_veiculo(client):
    # popula bd testes
    client.post("/tipos-veiculo/", json={"nome": "Moto", "tarifa_hora": 5.0})
    client.post("/tipos-veiculo/", json={"nome": "Carro", "tarifa_hora": 6.50})

    # requisição rota principal
    response = client.get("/tipos-veiculo")
...
def test_update_tipo_veiculo(client):
    ...
    response_update = client.put(f"/tipos-veiculo/{created_id}", json=update_data)
